    ...
    output: {
        assetFileNames: (assetInfo) => {
            ...
            if (name.endsWith('.css')) {
                return `_nuxt/css/${sectionFirts}_[hash]${version}[extname]`; // Carpeta y formato para archivos  css
            } else {
                return `_nuxt/assets/a${sectionFirts}_[hash]${version}[extname]`; // Carpeta y formato para archivos extension
            }
        },
        chunkFileNames: (chunkInfo) => {
            ...
            return `_nuxt/js/chunk_${name}.[hash]${version}.js`;
        },
        entryFileNames: () => {
            //Archivo js principal
            return `_nuxt/js/chunkMain__[name]-[hash]${version}.js`;
        },
    }
}